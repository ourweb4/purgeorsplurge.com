...
"use strict";
	...
	function prdctfltr_submit_form(curr_filter) {
		...
		if ( curr_filter.hasClass('prdctfltr_click_filter') || curr_filter.find('input[name="reset_filter"]:checked').length > 0 ) {
			...
			prdctfltr_respond_550(curr_filter);
	...
	$(document).on('click', 'span.prdctfltr_sale input, span.prdctfltr_instock input, span.prdctfltr_reset input', function() {
		...
		var checkShortcode = curr.closest('.prdctfltr_sc_products ');
		...
		if ( archiveAjax===true ) {
			...
			if ( typeof multiAjaxWidget == 'undefined' ) {
	...
	$(document).on('click', '.prdctfltr_filter_title a.prdctfltr_title_remove, .prdctfltr_regular_title a, .prdctfltr_widget_title a', function() {
		...
		var checkShortcode = curr_filter.closest('.prdctfltr_sc_products');
		...
		ourObj.each( function(i, obj) {
			...
			if ( curr_key == 's' ) {
				...
				if ( curr_deep === true && curr_els.length > 1 ) {
					...
					obj.find('.prdctfltr_attributes[data-filter="'+curr_key+'"] > input[type="hidden"]').each(function() { //, .prdctfltr_add_inputs input[name="product_cat"]
	...
	function prdctfltr_remove_empty_inputs_550(obj) {
		...
		obj.find('.prdctfltr_filter input[type="hidden"], .prdctfltr_filter input.pf_search').each(function() { //, .prdctfltr_add_inputs input[type="hidden"]:not([name="post_type"])
	...
	$(document).on('click', '.prdctfltr_sc_products '+prdctfltr.ajax_pagination_class+' a, body.prdctfltr-ajax '+prdctfltr.ajax_pagination_class+' a, .prdctfltr-pagination-default a, .prdctfltr-pagination-load-more a', function() {
		...
		prdctfltr_respond_550($('.prdctfltr_wc:first'));
						...
						$.each(response, function(n,obj2) {
							...
							if ( obj2.hasClass('prdctfltr_wc') ) {
								...
								if ( obj2.length<1 ) {
									...
									$(prdctfltr.ajax_count_class).html('No products found!');
		...
		if ( type=='append' ) {
			...
			if ( prdctfltr.ajax_animation == 'none' ) {
				...
				if ( typeof curr_products !== 'undefined' ) {
					...
					var v = curr_products.css('visibility', 'hidden'), cur = 0;
					...
					function pfRndIn() {
						...
						if(cur != v.length) setTimeout(pfRndIn, 100);
