	...
	class WC_Prdctfltr_Shortcodes {
		...
		public static function prdctfltr_respond_550() {
			...
			if ( $opt['pf_set'] == 'shortcode' ) {
				...
				$prdctfltr_global['ajax_js'] = $prdctfltr_global['pagefilters'][$prdctfltr_global['unique_id']]['args'];

				$prdctfltr_global['ajax_atts'] = $prdctfltr_global['pagefilters'][$prdctfltr_global['unique_id']]['atts_sc'];
			...
			if ( $opt['pf_mode'] == 'archive' ) {
				...
				if ( isset( $opt['pf_orderby_template'] ) ) {
					...
					if ( !$_GET['orderby'] && isset( $prdctfltr_global['active_filters']['orderby'] ) ) {
		...
		public static function make_query() {
			...
			$woocommerce_loop['columns'] = apply_filters( 'loop_shop_columns', ( $pf_col > 0 ? $pf_col : $columns ) );
		...
		public static function get_products( $products ) {
			...
			if ( isset( $prdctfltr_global['pagefilters'][$id]['atts'] ) ) {

				extract( $prdctfltr_global['pagefilters'][$id]['atts'] );
