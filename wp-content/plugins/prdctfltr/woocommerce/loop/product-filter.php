						...
						switch( $k ) {
							...
							case 'min_price' :
								...
								if ( isset( $pf_activated['min_price'] ) && $pf_activated['min_price'] !== '' ) {
									...
									echo ' / <span>' . strip_tags( wc_price( $min_price ) ) . ' - ' . strip_tags( wc_price( $max_price ) ) . '</span> <a href="#" class="prdctfltr_title_remove" data-key="byprice"><i class="prdctfltr-delete"></i></a>';
			...
			<?php
				...
				foreach ( $curr_elements as $curr_el ) :
								...
								if ( $curr_options['wc_settings_prdctfltr_disable_showresults'] == 'no' && ( $curr_styles[5] == 'prdctfltr_disable_bar' || isset( $prdctfltr_global['widget_search'] ) ) && isset($pf_activated['min_price']) && $pf_activated['min_price'] !== '' ) {
									...
									if ( $paste_title === true ) {
										...
										if ( isset( $pf_activated['max_price'] ) && $pf_activated['max_price'] !== '' ) {
											...
											$max_price = strip_tags( wc_price( $pf_activated['max_price'] ) );
					...
					case 'range' :
						...
						$adpt_rng = isset( $curr_options['wc_settings_prdctfltr_range_filters']['pfr_adoptive'][$p] ) ? $curr_options['wc_settings_prdctfltr_range_filters']['pfr_adoptive'][$p] : 'yes';

						$attr = $curr_options['wc_settings_prdctfltr_range_filters']['pfr_taxonomy'][$p];



						if ( $total !== 0 && $adpt_rng == 'yes' && ( isset( $output_terms ) && ( !isset( $output_terms[$attr] ) || isset( $output_terms[$attr] ) && empty( $output_terms[$attr]) ) === true ) ) {
				...
				<?php
					...
					default :
						...
						if ( !empty( $catalog_categories ) && !is_wp_error( $catalog_categories ) ) {
							...
							if ( empty( $curr_cat_selected ) && isset( $curr_fo['settings']['mode'] ) && $curr_fo['settings']['mode'] == 'subonly' && empty( $curr_cat_selected ) && isset( $prdctfltr_global['active_permalinks'][$curr_fo['filter']] ) ) {
							...
							if ( isset( $curr_fo['settings']['mode'] ) && substr( $curr_fo['settings']['mode'], 0, 3 ) == 'sub' ) {
								...
								foreach( $catalog_categories as $term ) {
									...
									if ( !in_array( $term->slug, $curr_cat_selected ) ) {
									...
									$curr_cat_ids_selected[] = $term->term_id;

									$remember_parent = $term->parent > 0 ? $term->parent : 0;
						...
						/*						if ( in_array( $selected, $curr_include ) ) {
								...
								<?php
									...
									foreach ( $catalog_categories as $term ) {
										...
										if ( $pf_has_children === true && isset( $curr_fo['settings']['mode'] ) && substr( $curr_fo['settings']['mode'], 0, 3 ) == 'sub' && !empty( $curr_cat_selected ) &&  $curr_id_found == false && !in_array( $term->term_id, $curr_cat_ids_selected ) ) {
										...
										if ( ( $chck_subonly = ( $curr_fo['settings']['mode'] ) && $curr_fo['settings']['mode'] == 'subonly' && !empty( $curr_cat_selected ) && ( ( $remember_parent == 0 && in_array( $decode_slug, $curr_cat_selected ) || ( $remember_parent > 0 && $remember_parent == $term->term_id ) ) ) ) === false ) {
...
<?php
	...
	$prdctfltr_global['true_id'] = $prdctfltr_global['unique_id'];
