	...
	class WC_Prdctfltr {
		...
		public static function make_global( $set, $query = array() ) {
			...
			if ( $stop === false ) {
				...
				$prdctfltr_global['taxonomies_data'] = array_filter( $taxonomies_data );

				$prdctfltr_global['active_taxonomies'] = array_filter( $taxonomies );

				$prdctfltr_global['active_misc'] = array_filter( $misc );

				$prdctfltr_global['range_filters'] = array_filter( $rng_terms );

				$prdctfltr_global['active_filters'] = array_merge( $prdctfltr_global['active_taxonomies'], $prdctfltr_global['active_misc'], array_filter( $rng_for_activated ) );
		...
		public static function prdctfltr_get_settings() {
			...
			if ( self::$settings['wc_settings_prdctfltr_shop_page_override'] !== '' && is_shop() ) {
			...
			if ( $curr_transient === false ) {
				...
				$pf_chck_settings = array(
					...
					'wc_settings_prdctfltr_adoptive_mode' => 'filter',
		...
		?>
			...
			<?php
				...
				if ( $curr_options['wc_settings_prdctfltr_disable_reset'] == 'no' && isset( $pf_activated ) && !empty( $pf_activated ) && $pf_activated !== $prdctfltr_global['active_permalinks'] ) {
